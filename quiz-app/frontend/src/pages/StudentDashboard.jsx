...
import { getStudentStats, getStudentHistory, getStudyPlan } from '../api/client';
...
export default function StudentDashboard() {
  ...
  const [studyPlan, setStudyPlan] = useState(undefined); // undefined = loading, null = none
  const [confirmLogout, setConfirmLogout] = useState(false);

  useEffect(() => {
    getStudentStats().then(setStats).catch(console.error);
    getStudentHistory().then(setHistory).catch(console.error);
    getStudyPlan().then(setStudyPlan).catch(() => setStudyPlan(null));
  }, []);
  ...
  return (
    <div className="page" style={{ padding: 0 }}>
      ...
      <div className="container" style={{ padding: '0 20px' }}>
        ...
        {/* AI Study Plan */}
        {studyPlan && (
          <div className="card" style={{ marginTop: 20, borderLeft: '4px solid #8b5cf6' }}>
            <p className="section-title" style={{ marginTop: 0, color: '#8b5cf6' }}>🤖 AI Study Plan</p>
            <p style={{ fontSize: '.78rem', color: 'var(--text-muted)', marginBottom: 10 }}>
              Generated by Claude after your last quiz · {new Date(studyPlan.created_at).toLocaleDateString()}
            </p>
        ...
        {studyPlan === null && (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '.85rem', marginTop: 16, padding: '12px', background: '#f8fafc', borderRadius: 10 }}>
            🤖 Complete a quiz while logged in — Claude will generate a personalised study plan for you
          </div>
        )}
